String.prototype.replaceAt = function(start, string) {
	return this.substr(0, --start) + string + this.substr(start + string.length);
};
...
(function(_c) {
    ...
    var swapArray = function(array, a, b){
        var removed = array.splice(b, 1, array[a]);
        array[a] = removed[0];
    };
            ...
            constructor: function(object) {
                ...
                if (Object.isArray(object)) {
                    for (var i=0; i < object.length; i++) {
                        this.items.push(new ClassType(object[i]));
                        this.keys.push(this.__counterId++);
                    }
                } else {
                    for (var key in object) {
                        this.items.push(new ClassType(object[key]));
                        this.keys.push(key);
                    }
                }
            },
...
(function () {
    ...
    EventBus = Class.CollectionOf(Event).create({
        ...
        off: function (eventName) {
            this.getCollection('name', eventName).each(function(index, key) {
                this.remove(key);
            }, this);
        },
    ...
    HtmlNode = Class.create({
        ...
        css: function(property, value) {
            if (typeof value !== "undefined") {
                ...
                var digit = window.getComputedStyle(this.htmlNode, null).getPropertyValue(property).match(/\d+/);
                if (digit) {
...
(function (namespace) {
    ...
    function getNodeValue(node) {
        var value = isNaN(node.textContent) ? node.textContent : parseFloat(node.textContent);
        return value;
    }
...
var models = new Packages('models'), Model;
var interfaces = new Packages('interfaces');

(function () {

    function addEventListener(item, action, callback, self) {
        item.__callbacks = item.__callbacks || [];
        item.__wrappedCallbacks = item.__wrappedCallbacks || [];
        item.__callbacks.push(callback);
        var wrapped = function (event) {callback.call(self, event);};
        item.__wrappedCallbacks.push(wrapped);
        item.addEventListener(action, wrapped);
    }

    function removeEventListener(item, action, callback) {
        var index = item.__callbacks.indexOf(callback);
        item.removeEventListener(action, item.__wrappedCallbacks[index]);
        item.__callbacks.splice(index,1);
        item.__wrappedCallbacks.splice(index,1);
    }

    /* Component */
    var componentBus = new EventBus();

    var HtmlNodeEvent = Class.extend(HtmlNode).create({
    ...
    Component = Class.extend(HtmlNodeEvent).CollectionOf(Object).create({
        constructor: function(htmlNode, model) {
            this.super();
            if (htmlNode) {
                ...
                this.htmlNode = this.__replaceTemplate(htmlNode);
                this.__parseNodes();
                this.__fillData();
        ...
        addBinding: function (node, attributeType, attributeValue) {
            this.bindings.add(new ItemBinding(node, attributeType, attributeValue));
        },
        __setTemplate: function(htmlNode) {
            var newNode = htmlNode.cloneNode(true);
            newNode.removeAttribute('data-component');
            newNode.removeAttribute('id');
            this.template = templates[this.componentName] || newNode.outerHTML;
        },
        __saveStaticData: function(htmlNode, model) {
            this.model = model || new Model({});
            if (!model && templates[this.componentName]) {
        ...
        __render: function() {
            ...
            if (this.componentName) {
                this.htmlNode.addClass('components', this.componentName);
            }
        },
        addListener: function(htmlNode, actionsList, callback) {
        ...
        addInterface: function(htmlNode, interfaceName) {
            interfaces.getPackage(interfaceName).call(this, htmlNode);
        }
    });

    var Item = Class.extend(HtmlNodeEvent).create({
        constructor: function(htmlNode) {
            this.super(htmlNode);
            if (htmlNode) {
                htmlNode.removeAttribute('data-item');
            }
        }
    });

    var ItemBinding = Class.extend(HtmlNodeEvent).create({
        constructor: function(htmlNode, type, itemName) {
            this.super(htmlNode);
            this.type = type;
            this.itemName = itemName;
            htmlNode.removeAttribute('data-bind');
        }
    });
    var Bindings = Class.CollectionOf(ItemBinding).create();

    var Collection = Class.CollectionOf(Component).create({
        constructor: function(container, htmlNode, componentName) {
            this.super();
            this.container = container;
            htmlNode.removeAttribute('data-collection');
            this.htmlNode = htmlNode;
        ...
        newItem: function (itemData) {
            ...
            var newItem = new Comp(newNode, itemData);
            this.add(newItem);
            return newItem;
        }
    });
    ...
    var parseNodes = function (mainNode, component) {
        var parseNode = function (node) {
            ...
            if (attributeValue) {
                ...
                addBind(node, attributeValue[0], attributeValue[1], component);
            }
    ...
    var addBind = function (node, attributeType, attributeValue, component) {
        node.setAttribute('ready','');
        component.addBinding(node, attributeType, attributeValue);
        if (!componentBus.get('model:' + attributeValue + ':change')) {
            component.on('model:' + attributeValue + ':change', component.modelChange);
        }
    };

    var addListener = function (node, attributeValue, component) {
        attributeValue = attributeValue.split(':');
        component.addListener(node, attributeValue[1], component[attributeValue[0]]);
    };
    ...
    var addComponent = function (node, attributeValue, component) {
        var pack = components.getPackage(attributeValue) || Component;
        component.add(new pack(node, component.model[attributeValue]), attributeValue);
        return false;
    };

    var addCollection = function (node, attributeValue, collection) {
        var templateName;
        if (attributeValue.indexOf(':') !== -1) {
            templateName = attributeValue.substr(0, attributeValue.indexOf(':'));
            attributeValue = attributeValue.substr(attributeValue.indexOf(':')+1);
        }
        collection.add(new Collection(node.parentNode, node, templateName), attributeValue);
    };
    ...
    Model = Class.CollectionOf(Object).create({
        ...
        set: function (key, newValue) {
            this.super(key, newValue);
            componentBus.trigger('model:' + key + ':change', key, this);
        },
        setAt: function(index, newValue) {
            var key = this.super(index, newValue);
            componentBus.trigger('model:' + key + ':change', key, this);
        },
    ...
    var ModelCollection = Class.CollectionOf(Model).create({
        constructor: function(data) {
            this.super(data);
        },
        newItem: function (itemData) {
